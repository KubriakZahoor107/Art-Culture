...
import { 
// authController,
getCurrentUser, login, register, registerUser, resetPassword, resetPasswordConfirm, updateUserProfile, } from "../controllers/authController.js";
import authenticateToken from "../middleware/authMiddleware.js";
import authorize from "../middleware/roleMIddleware.js";
import uploadProfileLogo from "../middleware/uploadProfileLogoImages.js";
const router = express.Router();
// Registration Route with Validation
router.post("/register", uploadProfileLogo.upload, (req, res, next) => {
    if (!req.file) {
        return next();
    }
...
router.post("/signup", authenticateToken, authorize("ADMIN"), [
    body("email").isEmail().withMessage("Enter a valid email"),
    body("password")
        .isLength({ min: 6 })
        .withMessage("Password must be at least 6 characters"),
...
router.put("/me", authenticateToken, uploadProfileLogo.upload, (req, res, next) => {
    if (!req.file) {
        return next();
    }
    uploadProfileLogo.processImages(req, res, next);
